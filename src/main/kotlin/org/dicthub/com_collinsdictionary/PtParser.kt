...
class PtParser : DictHtmlParser {
    ...
    private fun parseSenseLine(element: JQuery): DictItemSenseItemLine? {
        ...
        if (isBlank && element.has(".sense").length.toInt() == 0) {
            if (element.children(".re").has(".type-phr").length.toInt() > 0) {
                return parsePhrAsSenseList(element.children(".re").has(".type-phr"))
            }
        }
    ...
    private fun parsePhrAsSenseList(element: JQuery): DictItemSenseItemLine? {
        val orth = element.children(".type-phr").text()
        val translation = element.children(".type-translation").text()

        if (orth.isBlank() && translation.isBlank()) {
            return null
        }
        return DictItemSenseItemLine(
                translation = translation,
