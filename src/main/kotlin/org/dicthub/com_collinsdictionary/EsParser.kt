...
class EsParser : DictHtmlParser {
    ...
    private fun parseSenseListItem(element: JQuery): DictItemSenseItem? {
        ...
        val subLineElements = element.find(".sense")
        ...
        val phrases = element.find(".type-phr").mapToList { parseSenseItemPhrase(it) }
    ...
    private fun parseSenseLine(element: JQuery): DictItemSenseItemLine? {
        ...
        if (isBlank) {
            if (element.children(".re").has(".type-phr").length.toInt() > 0) {
                return parseSenseMinimalUnit(element.children(".re").has(".type-phr"))
            }
        }
